...
"""
Test script for LayoutLMv3-based Renaissance layout recognition model.
"""
...
def test_model(args):
    """
    Test the LayoutLMv3 model on a single image.
    ...
    # Load model
    print(f"Loading model from {args.model_path if os.path.exists(args.model_path) else 'pretrained LayoutLMv3'}")
    model = create_layoutlmv3_model(pretrained_path=args.model_path if os.path.exists(args.model_path) else None, device=device)
    model.eval()
    ...
    # Preprocess image
    image_tensor, _ = model.preprocess_image(args.image_path)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Test LayoutLMv3 Renaissance layout model')
    parser.add_argument('--model_path', type=str, default='models/layoutlmv3/best_model.pth',
                        help='Path to trained model (optional, will use pretrained if not found)')
    parser.add_argument('--image_path', type=str, default='data/renaissance/val/1.png',
                        help='Path to test image')
    parser.add_argument('--output_dir', type=str, default='results/layoutlmv3_test',
